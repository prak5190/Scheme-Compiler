(library (Compiler convert-assignments)
  ...
  (define-who (convert-assignments program)
    (define (Exp exp ls)                   ;get-trace-define
      (match exp
        ((,y (lambda (,x ...) (assigned ,als ,z))) (let* ((new-x (map (lambda(x)
                                                                     (if (memq x als)
                                                                         (unique-name (string->symbol (extract-root x)))
                                                                         x)) x))
                                                          (cons-ls (fold-left (lambda (s x y) (if (eqv? x y) s (cons `(,x (cons ,y (void))) s)))
                                                                              '() x new-x))
         ...
         ;; (let* ((new-x (map (lambda(x) (unique-name (string->symbol (extract-root x)))) x))
      ...
      (let* ((OExpr Expr)
        ...
        (match exp
          ...
          ((let ((,x ,y) ...) (assigned ,als ,z)) (let* ((y (map (lambda(y) (Expr y)) y))                                                         
                                                         (tx-ls (map (lambda(y x)
                                                                       (if (memq x als)
                                                                           `(,(unique-name (string->symbol (extract-root x))) ,y)
                                                                           `(,x ,y))) y x))
                                                         (cons-ls (fold-left (lambda (s x y)
                                                                               (if (eqv? x y) s (cons `(,x (cons ,y (void))) s)))
                                                                             '() x (map car tx-ls)))
          ...
          ((letrec ((,x ,[Expr -> z]) ...) ,[Expr -> y]) (let ((rls (map (lambda(x z) `(,x ,z)) x z)))
                                                           `(letrec ,rls ,y)))
          ((lambda (,x ...) (assigned ,als ,z)) (let* ((new-x (map (lambda(x)
                                                                     (if (memq x als)
                                                                           (unique-name (string->symbol (extract-root x)))
                                                                           x)) x))
                                                       (cons-ls (fold-left (lambda (s x y) (if (eqv? x y) s (cons `(,x (cons ,y (void))) s)))
