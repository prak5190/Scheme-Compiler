  ...
  (define-who (normalize-context program)
    ...
    (define (Value exp)
      (match exp
        ...
        ((let ((,x ,y) ...) ,z) (let* ((ls (map (lambda(x y) `(,x ,y)) x (Value* y))))
                                  `(let ,ls ,(Value z))))
        ((quote ,x) exp)
        ((,x ,[Value -> y] ...) (guard (or (value-prim? x) (label? x))) `(,x ,y ...))
        ((,x ,[Value -> y] ...) (guard (pred-prim? x)) `(if (,x ,y ...) '#t '#f))        
        ;; Any other prim makes no sense , just take the values       
        ((,x ,[Value -> y] ...) (guard (prim? x)) (make-nopless-begin y))
        ((,x ...) (Value* x))
        (,x (guard (var? x)) x)
        (,else `(nop))))
    ...
    (define (Pred exp)
      (match exp
        ...
        ((,x ,[Value -> y] ...) (guard (or (value-prim? x) (label? x))) `(if (eq? (,x ,y ...) '#f) (false) (true)))
        ((,x ,[Value -> y] ...) (guard (prim? x)) `(,(make-nopless-begin y) ... (true)))
