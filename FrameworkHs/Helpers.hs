...
data P423Pass a b =
  P423Pass
    { pass :: a -> PassM b               -- ^ The implementation of the pass
...
hPutGenM c g h = case runReaderT (runWriterT g) c of
  ...
  Right (_, b) -> hPut h $ toByteString b
...
runGenM c g = case runReaderT (runWriterT g) c of
  Left s -> Left s
  Right (x, bu) -> Right (x, toByteString bu)
...
catchPassM :: PassM a -> PassM a -> PassM a
catchPassM m1 m2 = catchError m1 (const m2)
